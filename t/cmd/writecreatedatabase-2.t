...
"$rootdir/perl" "$rootdir/bin/dango.pl" "$rootdir/sketch/define-storage-3.txt" \
    --fill-instance-prop=name,table_stem,timeline_type \
    --write-create-database-single-text "$tempdir/create.sql" 2> /dev/null

(diff -u "$tempdir/create.sql" "$rootdir/sketch/define-storage-3-create.sql" && echo "ok 1") || echo "not ok 1"
rm -fr "$tempdir"
